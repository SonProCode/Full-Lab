...
# Python builds linked to older OpenSSL do not accept ReportLab's
# usedforsecurity keyword. Keep the workaround local to this report tool.
...
SOURCE = ROOT / "reports/sre-full-lab-report.md"
OUTPUT = ROOT / "reports/sre-full-lab-report.pdf"
...
def footer(canvas, doc):
    ...
    canvas.drawString(18 * mm, 10 * mm, "Production Backend Lab - Báo cáo thực hành SRE")
...
def build_story(text, styles):
    ...
    for line in lines:
        ...
        if match:
            ...
            story.extend([Paragraph(inline_markup(match.group(2)), styles[f"h{level}"]), Spacer(1, 2 * mm)])
            continue
...
def main():
    ...
    styles = {
        "h1": ParagraphStyle(
            ...
            leading=26, textColor=colors.HexColor("#0F4C81"), alignment=TA_CENTER,
            spaceAfter=8 * mm,
        ),
        "h2": ParagraphStyle(
        ...
        "meta": ParagraphStyle(
            "Meta", parent=sample["BodyText"], fontName="NotoSans", fontSize=9,
            leading=13, alignment=TA_LEFT, textColor=colors.HexColor("#475569"),
        ),
        ...
        "code": ParagraphStyle(
            "Code", parent=sample["Code"], fontName="NotoSans", fontSize=7.6,
        ...
        "table": ParagraphStyle(
            "Table", parent=sample["BodyText"], fontName="NotoSans", fontSize=7.7,
    ...
    doc = BaseDocTemplate(
        ...
        title="Báo cáo thực hành SRE - Production Backend Lab Full Mode",
